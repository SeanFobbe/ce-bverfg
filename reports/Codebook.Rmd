...
```{r, CE-BVerfG_Workflow_Graph_blackwhite, fig.width = 12, fig.height = 14, fig.pos = "p", fig.cap = "Der vollständige Prozess der Datensatz-Kompilierung."}
...
# Zitationsnetzwerk des Bundesverfassungsgerichts (Beta)

## Überblick

Der Datensatz enthält zusätzlich eine spezialisierte Variante, die Zitate des BVerfG aus den Entscheidungstexten zu seiner eigenen Rechtsprechung extrahiert und in strukturierter Form aufbereitet. Das Verfahren orientiert sich an Coupette, *Juristische Netzwerkforschung* (Mohr Siebeck 2019), pp. 241--244, benutzt aber andere reguläre Ausdrücke, um die Erkennung von langen Blöcken mit Rechtsprechungszitaten zu verbessern.

Diese Variante ist noch in der Beta-Testphase. Folgende Rechtsprechungszitate sind enthalten:

- Zitate innerhalb der amtlichen Sammlung (inklusive langer Blockzitate mehrer BVerfGE-Entscheidungen)
- Zitate von amtlicher Sammlung zu Aktenzeichen
- Zitate von Aktenzeichen zu Aktenzeichen

Zitate unter Angabe des Aktenzeichens sind weniger genau als Zitate zu konkreten Entscheidungen (bei denen Datum und ggf. Kollisionsziffer nötig sind), aber sind eine gute Näherung. Zitate konkreter Entscheidung unter Angabe des Datums sind für die Zukunft vorgesehen, benötigen aber noch zusätzlichen Entwicklungsaufwand.

**Achtung:** Zitierende Entscheidungen können nur solche sein, für die im Korpus der Volltext enthalten ist. Zitierte Entscheidungen können alle aus der gesamten Rechtsprechung sein (keine Begrenzung).

Das Zitationsnetzwerk wird als GraphML-Datei angeboten und kann z.B. einfach in graphische Softwre wie Gephi importiert werden. Formal handelt sich um einen gewichteten, gerichteten Graphen (Digraph). Die Anzahl der Knoten gibt die Anzahl der Entscheidungen/Aktenzeichen mit eingehenden und/oder ausgehenden Zitaten an. Die Anzahl der Kanten gibt die Anzahl der Entscheidungen/Aktenzeichen-Paare mit mindestens einem Zitat an. Die Gewichte für die Kanten geben die Anzahl der Zitate an. Die Ausgangsstärke gibt die gesamte Zahl aller einfachen Zitate an.
...
dt.g.split <- cbind(dt.g[1:8], dt.g[9:16])
...
In der **ersten Stufe** werden die Zitierblöcke lokalisiert und aus dem Volltext gesammelt. Es wird die starke Annahme getroffen, dass Zitierblöcke mit \enquote{BVerfGE} (ignoriert Groß- und Kleinschreibung) eingeleitet werden und nur whitespace, Zahlen, gewisse Sonderzeichen und gewisse Buchstaben enthalten. Zitierblöcke enden in der Regel mit einer runden Klammer, die in der REGEX nicht enthalten ist um sie als Grenzzeichen zu nutzen.

Die konkrete regular expression (REGEX) ist die folgende:
...
In der **zweiten Stufe** werden aus allen Zitierblöcken die einzelnen Zitate extrahiert, standardisiert und mit der Ausgangsentscheidung verbunden. Die Extraktion trifft die starke Annahme, dass eine Entscheidung der amtlichen Sammlung entweder mit \enquote{BVerfGE} oder bei einem Mehrfachzitat in einem Zitierblock mit einem Semikolon eingeleitet wird. Folgende REGEX kommt dabei zum Einsatz:
...
print(regex.bverfge.cite)
```

Damit findet man zwei Mögliche Einzelzitate:

- \enquote{BVerfGE 5, 85}
- \enquote{; 59, 231}

Die Einzelzitate werden in einem späteren Schritt bereinigt und standardisiert. Zum Ende hin werden Selbstzitate entfernt und Metadaten hinzugefügt.
...
- Verkürzte Schreibweise wie in BVerfGE 60, 162: \enquote{BVerfGE 3, 19 (27), 383 (394); 4, 375 (381 f.);} --- das Beispiel stammt aus Coupette (2019: 246)
...
Coupette (2019) nutzt in der ersten Stufe eine REGEX, die strenger ist und die konkrete Form des Zitats (3 Nummern gefolgt von Komma und 3 Nummern) berücksichtigt. In der zweiten Stufe entfernt Coupette (2019) erst Inhalte in spitzen Klammern und extrahiert dann die Einzelzitate. Meine Vorgehensweise ist in der ersten Stufe liberaler, in der zweiten Stufe strenger. 

Aktuell ist unklar, welche Strategie insgesamt besser ist, da die Prüfung auf falsch-negative und falsch-positive Fehler bei Coupette (2019: 245) N=33 Entscheidungen per Hand umfasste und ich bisher nur informell falsch-positive Fehler und falsch-negative Fehler gar nicht geprüft habe.



## Methodik Aktenzeichen

Über Coupette (2019) hinaus enthält dieser Datensatz auch zitierte Aktenzeichen (Aktenzeichen-Aktenzeichen-Zitate) und Zitate von nicht-BVerfGE-Aktenzeichen zu BVerfGE-Entscheidungen (Aktenzeichen-BVerfGE-Zitate). 

Diese sind einfacher zu erfassen und werden mit folgender REGEX gesucht:
...
Um konkrete Entscheidungen zu zitieren müsste zusätzlich zum Aktenzeichen noch das Datum berücksichtigt werden. Weil dies die REGEX deutlich komplizierter macht, ist dieser Schritt noch ni Arbeit. Im Datensatz sind allerdings `r round(uniqueN(dt.bverfg.meta$aktenzeichen)/dt.bverfg.meta[,.N] * 100, digits = 2)` % aller ausgehenden Aktenzeichen einzigartig (unabhängig vom Datum), sodaß das Aktenzeichen eine gute Näherung darstellt.

Falls eine BVerfGE-Entscheidung eine BVerfGE-Entscheidung zitiert hat diese Zitierweise absoluten Vorrang. Für BVerfGE-BVerfGE-Zitate besteht im Datensatz daher eine direkte Entscheidung-Entscheidung-Relation.
...
```{r, CE-BVerfG_Zitationsnetzwerk_Knotengrad, fig.width = 9, fig.height = 6, fig.cap = "Verteilung der Knotengrade im Zitationsnetzwerk des BVerfG."}
...
```{r, CE-BVerfG_Zitationsnetzwerk_Eingangsgrad, fig.width = 9, fig.height = 6, fig.cap = "Verteilung der Eingangsgrade im Zitationsnetzwerk des BVerfG."}
...
```{r, CE-BVerfG_Zitationsnetzwerk_Sugiyama_schwarz, fig.width = 40, fig.height = 50, fig.dpi = 50, fig.pos = "p", fig.cap = "Das Zitationsnetzwerk des BVerfG ab 1998", dev = "png"}
...
ggraph(g, "sugiyama") + 
    ...
    labs(
        title = paste(prefix.figuretitle,
                      "| Zitationsnetzwerk BVerfGE-zu-BVerfGE ab 1998"),
        caption = caption
    )+
    theme(
...
```{r, CE-BVerfG_Density_Zeichen, fig.height = 6, fig.width = 9}
...
```{r,  CE-BVerfG_Density_Tokens, fig.height = 6, fig.width = 9}
ggplot(data = dt.bverfg.meta)+
...
```{r, CE-BVerfG_Density_Typen, fig.height = 6, fig.width = 9}
ggplot(data = dt.bverfg.meta)+
...
```{r, CE-BVerfG_Density_Saetze, fig.height = 6, fig.width = 9}
ggplot(data = dt.bverfg.meta)+
...
```{r, CE-BVerfG_Barplot_Entscheidungstyp, fig.height = 6, fig.width = 9}
...
```{r, CE-BVerfG_Barplot_Spruchkoerper_Typ, fig.height = 6, fig.width = 9}
...
```{r, CE-BVerfG_Barplot_Spruchkoerper_AZ, fig.height = 6, fig.width = 9}
...
```{r, CE-BVerfG_Barplot_Registerzeichen, fig.height = 10, fig.width = 8}
...
```{r, CE-BVerfG_Barplot_PraesidentIn, fig.height = 6, fig.width = 9}
...
```{r, CE-BVerfG_Barplot_VizePraesidentIn, fig.height = 6, fig.width = 9}
...
```{r, CE-BVerfG_Barplot_Entscheidungsjahr, fig.height = 6, fig.width = 9}
ggplot(data = freqtable$entscheidungsjahr[-.N]) +
    geom_bar(aes(x = as.integer(entscheidungsjahr),
                 y = N),
             stat = "identity",
             fill = "#ca2129") +
    theme_bw() +
...
```{r, CE-BVerfG_Barplot_EingangsjahrISO, fig.height = 6, fig.width = 9}
ggplot(data = freqtable$eingangsjahr_iso[-.N]) +
    geom_bar(aes(x = as.integer(eingangsjahr_iso),
                 y = N),
             stat = "identity",
             fill = "#ca2129") +
...
```{r, CE-BVerfG_Density_Dateigroessen_PDF, fig.height = 6, fig.width = 9}
...
```{r, CE-BVerfG_Density_Dateigroessen_TXT, fig.height = 6, fig.width = 9}
...
```{r, CE-BVerfG_Density_Dateigroessen_HTML, fig.height = 6, fig.width = 9}
